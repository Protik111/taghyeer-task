...
export default function Button({
  ...
  if (href) {
    // `disabled` only affects the button-mode branch below — a link with
    // an onClick handler would force this Server-Component-safe file to
    // draw a client boundary just to guard a case no caller uses (no
    // `href` Button in this app is ever rendered disabled). The
    // `pointer-events-none` class above is enough if that changes.
    return (
